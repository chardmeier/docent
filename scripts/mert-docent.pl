...
# $Id$
# Usage:
# mert-docent.pl <foreign> <english> <decoder-executable> <decoder-config>
# For other options see below or run 'mert-moses.pl --help'

# Notes:
# <foreign> and <english> should be
#    for Moses: raw text files, one sentence per line
#    for Docent: xml files
# <english> can be a prefix, in which case the files are <english>0, <english>1, etc. are used

# Excerpts from revision history

# Jul 2013    adapted for usage with Docent (Sara Stymne)
...
my @ABBR_FULL_MAP = qw(d=weight-d d1=weight-d1 d2=weight-d2 d3=weight-d3 d4=weight-d4 d5=weight-d5 d6=weight-d6 d8=weight-d8 d10=weight-d10  lm=weight-l lm2=weight-l2 tm=weight-t w=weight-w  g=weight-generation lex=weight-lex I=weight-i cqp=weight-cqp cqw=weight-cqw tt=weight-tt ov=weight-ovix lw=weight-longword xlw=weight-xlongword oov=weight-oov );
...
my $__DOCENT = 0;
my $__DOCENT_SAVE_STATE = 0;

## options for forking 
my $__SPLIT_FORK = 0; # number of sentences in each translation batch, when using forked decoding
my $SPLIT_FORK_CHUNK = 0;  # number of chunks when using forked decoding
my $NO_SPLIT_FILE = 0; # if running forked but file is already split

##NEW VARIABLES FOR POSTPROCESSING
my $post_process = 0;  #by default no postprocessing
my $post_process_script = undef; 
...
GetOptions(
  ...
  "threads=i" => \$__THREADS,
  "docent" => \$__DOCENT,
  "docent-save-state" => \$__DOCENT_SAVE_STATE,
  "split-fork=i" => \$__SPLIT_FORK, # number of sentences in each translation batch, when using forked decoding (default = 0 , i.e. no forked decoding)
  "no-split-file=i" => \$NO_SPLIT_FILE, 
  "postprocess" => \$post_process,
  "postprocess_script=s" => \$post_process_script,  
) or exit(1);
...
Options:
  ...
  --postprocess ... Perform a postprocessing step
  --postprocess_script=STRING ... use this script for postprocessing (assuming it reads from standard in and writes to standard out
";
  exit 1;
}

##checking if postprocessing stuff OK
die "You have to specify a postprocessing script in order to run postprocessing"
    if $post_process && !defined $post_process_script;
...
# split input file, if forking is on
if ($__SPLIT_FORK) {
    if (!$NO_SPLIT_FILE) {
	$SPLIT_FORK_CHUNK = split_input_file($___DEV_F, $__SPLIT_FORK);
    }
    else {
	$SPLIT_FORK_CHUNK = $NO_SPLIT_FILE;	
    }
    #my $split_cmd = "split -a 1 -l $__SPLIT_FORK_CHUNK -d $___DEV_F $___DEV_F.";
    #safesystem ($split_cmd) or die("Couldn't split the input file into chunks for forking");
}
...
if ($__DOCENT_SAVE_STATE) {
    $___DECODER_FLAGS .= " -p statefile";
}
...
my $featlist;
if ($__DOCENT) {
    $featlist = get_featlist_from_docent($___CONFIG);
}
else {
    $featlist = get_featlist_from_moses($___CONFIG);
}
...
if (defined $___ACTIVATE_FEATURES) {
	if ($___ACTIVATE_FEATURES =~ /^_/) { #new format "_0,_1,..." -- starts with '_'
		for (my $i = 0; $i < scalar(@{$featlist->{"names"}}); $i++) {
			if ($___ACTIVATE_FEATURES =~ /_$i\b/) {
				$featlist->{"enabled"}->[$i] = 1;
			}
			else {
				$featlist->{"enabled"}->[$i] = 0;
			}
		}
	} #old format "w_0,lm_0,..."
	else {
...
while (1) {
  ...
  # create config that reads state, if saved states used
  if ($__DOCENT_SAVE_STATE && $run == 2) {
      print STDERR "updating config file with saved state, run $run\n";
      update_docent_config_state($___CONFIG,$___CONFIG."state");
      $___CONFIG = $___CONFIG."state";
  }
  ...
  if (! $skip_decoder) {
	...
	# keep normalizing if activate features used, since Mert does not do that in that case
	$need_to_normalize = 0 if !(defined $___ACTIVATE_FEATURES);
...
sub run_decoder {
    ...
    if ($need_to_normalize) {
		if (defined $___ACTIVATE_FEATURES) {
			...
			my $totlambda = 0;
			for (my $i=0; $i<@vals;$i++) {
				$totlambda += abs($vals[$i]) if $featlist->{"enabled"}->[$i];
			}
			for (my $i=0; $i<@vals;$i++) {
				$vals[$i] /= $totlambda if $featlist->{"enabled"}->[$i];
			}
    ...
    my $decoder_config;
    my %weight_store; #TODO - fix and remove this, just used temporarily

    if ($__DOCENT) {
	#TODO: RMOVE THE BELOW AND FIX IN A GOOD WAY!!
	for(my $i=0; $i<scalar(@{$featlist->{"names"}}); $i++) {
	    my $name = $featlist->{"names"}->[$i];
	    $weight_store{$name} = [] if !defined $weight_store{$name};
	    push @{$weight_store{$name}}, sprintf "%.6f", $vals[$i];
	}

    # 	# TODO update this to fit new parameter modification system!!
    # 	my $num = 0;
    # 	my $name;
    # 	for(my $i=0; $i<scalar(@{$featlist->{"names"}}); $i++) {
    # 	    if ($name ne $featlist->{"names"}->[$i]) {
    # 		$name = $featlist->{"names"}->[$i];
    # 		$num = 0;
    # 	    }
    # 	    $decoder_config .= " -weight $name $vals[$i] $num";
    # 	    $num++;
    # 	}
    }
    my %model_weights;
    ...
    $decoder_config = join(" ", values %model_weights);
    ...
    if ($post_process) {
	$nbest_outfile = $filename."temp";
	$post_process_cmd = "$post_process_script < $nbest_outfile > $filename";
    }

    if (defined $___JOBS && $___JOBS > 0) {
	die ("Docent not supported with parallel") if $__DOCENT;
      $decoder_cmd = "$moses_parallel_cmd $pass_old_sge -config $___CONFIG -inputtype $___INPUTTYPE -qsub-prefix mert$run -queue-parameters \"$queue_flags\" -decoder-parameters \"$___DECODER_FLAGS $decoder_config\" $lsamp_cmd -n-best-list \"$nbest_outfile $___N_BEST_LIST_SIZE\" -input-file $___DEV_F -jobs $___JOBS -decoder $___DECODER > run$run.out";
    safesystem($decoder_cmd) or die "The decoder died. CONFIG WAS $decoder_config \n";

    }  elsif ($__SPLIT_FORK) {
	die ("Docent not supported with split-fork") if $__DOCENT;
	my @childs ;
	for(my $i=10; $i < $SPLIT_FORK_CHUNK+10; $i++) {
	    my $pid = fork();
	    if ($pid) {
            # parent
		push(@childs, $pid);
		...
		# child
		$decoder_cmd = "$___DECODER $___DECODER_FLAGS  -config $___CONFIG -inputtype $___INPUTTYPE $decoder_config -n-best-list $nbest_outfile.$i $___N_BEST_LIST_SIZE -input-file $___DEV_F.$i > run$run.$i.out";
		safesystem($decoder_cmd) or die "The decoder died. Chunk $i, CONFIG WAS $decoder_config \n";
		exit(0);
	    } else {
		die "couldn’t fork:\n";
	    }
	...
	## cat separated file parts!!!
	## safesystem("cat run$run.?.out > run$run.out") or die("couldn't merge file parts after decoding\n");
	merge_nbest($nbest_outfile, $SPLIT_FORK_CHUNK);
	#safesystem("$mertdir/merge-nbest.perl --file $filename --num $__SPLIT_FORK > $filename")  or die("couldn't merge file parts of nbestlist after decoding\n");
    } else {
 #     $decoder_cmd = "$___DECODER $___DECODER_FLAGS  -config $___CONFIG -inputtype $___INPUTTYPE $decoder_config $lsamp_cmd -n-best-list $filename $___N_BEST_LIST_SIZE -input-file $___DEV_F > run$run.out";
	...
	if ($__DOCENT) {
	    update_weights_docent_config("./run$run.moses.ini", "./run$run.moses-weighted.ini", \%weight_store);
	    $decoder_cmd = "$___DECODER -i $___DEV_F -c ./run$run.moses-weighted.ini -o run$run -w $___WORKING_DIR $___DECODER_FLAGS";
	}
	else {
	    $decoder_cmd = "$___DECODER $___DECODER_FLAGS  -config ./run$run.moses.ini  -inputtype $___INPUTTYPE  $decoder_config  $lsamp_cmd -n-best-list $filename $___N_BEST_LIST_SIZE -input-file $___DEV_F > run$run.out"; 
	}
    ...
    if ($post_process) {
	die "post processing not supported for Docent" if ($__DOCENT);
	safesystem($post_process_cmd) or die "The post processing died. COMMAND WAS $post_process_cmd\n";
    }
...
sub get_featlist_from_docent {
  # run Docent with the given config file and return the list of features and
  # their initial values
  my $configfn = shift;
  my $featlistfn = "./features.list";
#  if (-e $featlistfn && ! -z $featlistfn) {   # exists & not empty
#   print STDERR "Using cached features list: $featlistfn\n";
# } else {
#Parse docent config file for feature functions + weights
      open CFILE, $configfn;
      open FFILE, ">$featlistfn";
  my $ind = 0;
      while (<CFILE>) {
	  if (/<weight.*model="(\w+)".*>([\d\.e-]+)</) {
	      print FFILE "$1 $1 $2\n";
...
sub create_docent_config {
  ...
  # NOT CURRENTLY USED FOR DOCENT. TODO: UPDATE???
  # # first convert the command line parameters to the hash
  # # ensure local scope of vars
  # {
  #   my $parameter = undef;
...
# ADD THESE BACK???
  ...
  while(my $line = <$ini_fh>) {
      if ($line =~ /<weight model="(\w+)"(.*>)([\d\.e-]+)(<.*)/) {
	  if (defined($P{$1})) {  #found the model
	      if (!defined($done{$1})) {  #haven't treated it yet
		  if (@{$P{$1}} > 1) {
		      my $num = 0;
		      foreach my $n (@{$P{$1}}) {
			  print $out "<weight model=\"$1\" score=\"$num\">$n</weight>\n";
			  $num++;
		      }
		  }
		  else {
		      print $out "<weight model=\"$1\">@{$P{$1}}[0]</weight>\n";
		  }
		  $done{$1} = 1;
		  next;
	      }
...
sub update_weights_docent_config {
  my $infn                = shift; # source config
  my $outfn               = shift; # where to save the config
  my $P          = shift; # the lambdas we should write
...
sub update_docent_config_state {
  my $infn                = shift; # source config
  my $outfn               = shift; # where to save the config
  ...
  while(my $line = <$ini_fh>) {
	  ...
	  if ($line !~ /\/>$/) {
	      while ($line = <$ini_fh>) {
		  last if $line =~ /<\/initial-state/
	      }
	  }
	  print $out "<initial-state type=\"saved-state\">\n".
	      "<p name=\"file\">statefile</p>\n".
	      "</initial-state>\n";
	  next;
      }     
      print $out $line;
...
sub split_input_file {
    ...
    open IN, "$FILE" or die("problem opening file: $FILE, when splitting input file");
    open OUT, "> $FILE.$fnum";
...
sub merge_nbest {
    ...
    open OUT, ">$FILE_PREFIX" or die("Couldn't open nbest-file for writing: $FILE_PREFIX");
    
    for (my $i = 0; $i < $NUM; $i++) {
	open IN, "$FILE_PREFIX.$i" or die("problem opening file: $FILE_PREFIX.$i, when merging split nbest list");
