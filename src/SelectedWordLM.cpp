...
class SelectedWordLM : public FeatureFunction {
...
FeatureFunction *SelectedWordLMFactory::createNgramModel(const Parameters &params) {
...
template<class M>
inline Float SelectedWordLM<M>::scoreNgram(const StateType_ &state,
...
template<class M>
FeatureFunction::State *SelectedWordLM<M>::initDocument(const DocumentState &doc, Scores::iterator sbegin) const {
...
template<class M>
void SelectedWordLM<M>::computeSentenceScores(const DocumentState &doc, uint sentno, Scores::iterator sbegin) const {
	*sbegin = Float(0);
}
...
template<class M>
FeatureFunction::StateModifications *SelectedWordLM<M>::updateScore(const DocumentState &doc, const SearchStep &step, const State *state,
		FeatureFunction::StateModifications *estmods, Scores::const_iterator psbegin, Scores::iterator estbegin) const {
	return estmods;
}

template<class M>
FeatureFunction::State *SelectedWordLM<M>::applyStateModifications(FeatureFunction::State *oldState, FeatureFunction::StateModifications *modif) const {
