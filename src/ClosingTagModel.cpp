...
struct ClosingTagModelState : public FeatureFunction::State, public FeatureFunction::StateModifications {
  ClosingTagModelState(uint nsents)  : logger_("ClosingTagModel") {}
  ...
  virtual ClosingTagModelState *clone() const {
    return new ClosingTagModelState(*this);
  }
};
...
ClosingTagModel::ClosingTagModel(const Parameters &params) :
  logger_("ClosingTagModel") {
...
FeatureFunction::State *ClosingTagModel::initDocument(const DocumentState &doc, Scores::iterator sbegin) const {
...
void ClosingTagModel::computeSentenceScores(const DocumentState &doc, uint sentno, Scores::iterator sbegin) const {
	*sbegin = Float(0);
}



FeatureFunction::StateModifications *ClosingTagModel::estimateScoreUpdate(const DocumentState &doc, const SearchStep &step, const State *state,
...
FeatureFunction::StateModifications *ClosingTagModel::updateScore(const DocumentState &doc, const SearchStep &step, const State *state,
		FeatureFunction::StateModifications *estmods, Scores::const_iterator psbegin, Scores::iterator estbegin) const {
	return estmods;
}

FeatureFunction::State *ClosingTagModel::applyStateModifications(FeatureFunction::State *oldState, FeatureFunction::StateModifications *modif) const {
	ClosingTagModelState *os = dynamic_cast<ClosingTagModelState *>(oldState);
	ClosingTagModelState *ms = dynamic_cast<ClosingTagModelState *>(modif);
