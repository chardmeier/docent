...
PhrasePairCollection::PhrasePairCollection(const PhraseTable &phraseTable, uint sentenceLength, Random random)
	: logger_(logkw::channel = "PhrasePairCollection"),
...
void PhrasePairCollection::addPhrasePair(CoverageBitmap cov, PhrasePair phrasePair) {
	BOOST_LOG_SEV(logger_, verbose) << "addPhrasePair " << cov << " " <<
...
bool PhrasePairCollection::proposeSegmentationLeftRight(const CoverageBitmap &range,
	...
	BOOST_LOG_SEV(logger_, verbose) << "proposeSegmentation " << range;

	if(range.none()) {
		BOOST_LOG_SEV(logger_, debug) << "Range empty.";
		return true;
	}

	if(startit == endit) {
		BOOST_LOG_SEV(logger_, debug) << "No phrase pairs available.";
		return false;
	}

	BOOST_LOG_SEV(logger_, debug) << "                    " << startit->first;
	...
	if(it1 == endit) {
		BOOST_LOG_SEV(logger_, debug) << "firstBit = " << firstBit;
		BOOST_LOG_SEV(logger_, debug) << "startit->find_first() = " << startit->first.find_first();
		BOOST_LOG_SEV(logger_, debug) << "No matching phrase pair.";
		return false;
	}
	...
	do {
		if(badChoices.count() == noptions) {
			BOOST_LOG_SEV(logger_, debug) << "Exhausted all choices.";
			return false;
		}
		...
		if((ph->first & range) != ph->first) { // are we covering something we shouldn't?
			BOOST_LOG_SEV(logger_, debug) << "Conflicting coverage:";
			BOOST_LOG_SEV(logger_, debug) << ph->first;
			BOOST_LOG_SEV(logger_, debug) << range;
			continue;
		}

		BOOST_LOG_SEV(logger_, debug) << "selected            " << ph->first;
		CoverageBitmap::size_type i = firstBit, next;
		BOOST_LOG_SEV(logger_, debug) << "First bit: " << i;
		while((next = ph->first.find_next(i)) == i + 1) // find first gap in phrase
			i = next;
		i++;
		BOOST_LOG_SEV(logger_, debug) << "Next unset bit: " << i;
	...
	BOOST_LOG_SEV(logger_, debug) << "Proposing " << *ph;
	seg.push_front(*ph);
	return true;
}
