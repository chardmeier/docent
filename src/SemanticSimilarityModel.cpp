...
struct SelectedWordVector {
  SelectedWordVector(uint pn, uint wn, std::string s, std::string t, uint sn, uint size)  : 
    srcWord(""), trgWord(""), phrNo(0), wordNo(0), srcNo(0), vec(0), similarity(0) 
  {
    phrNo = pn;
    wordNo = wn;
    srcWord = s;
...
struct SemanticSimilarityModelState : public FeatureFunction::State, public FeatureFunction::StateModifications {
  SemanticSimilarityModelState(uint nsents)  : logger_("SemanticSimilarityModel"), currentScore(0) {
  ...
  void LoadVectors(const std::string file_name){
    ...
    for (b = 0; b < words; b++) {
      ...
      while (1) {
        ...
        if (feof(f) || (vocab[b * MaxWordSize + a] == ' ')) break;
  ...
  long long FindVocabularyPosition(const std::string word){
    long long b;
    for (b = 0; b < words; b++){
  ...
  std::vector<float> FindVector(const std::string word,std::vector<float> &vec){
    vec.clear();
    long long b = FindVocabularyPosition(word);
    if (b>=0){
      for (int a = 0; a < size; a++) vec.push_back(M[a + b * size]);
    }
    return vec;
  }

  std::vector<float> FindVector(const long long b,std::vector<float> &vec){
    vec.clear();
    for (int a = 0; a < size; a++) vec.push_back(M[a + b * size]);
    return vec;
  }
  ...
  void GetHistory(std::vector<SelectedWordVector*> &history,uint sentNo, const uint size){
  ...
  void GetHistory(std::vector<SelectedWordVector*> &history,
		 uint sentNo, const uint idx, const uint size){

    std::vector<SelectedWordVector>::iterator it=selectedWords[sentNo].begin()+idx;
    while ( history.size() < size ){
      while (it!=selectedWords[sentNo].begin()){
	it--;
	history.push_back(&(*it));
  ...
  void GetFuture(std::vector<SelectedWordVector*> &future,
		 uint sentNo, const uint idx, const uint size,
  ...
  void UpdateFutureScores(uint sentNo, uint idx, const uint size,
                          const uint stopSentNo, const uint stopPhrNo){
    uint updated = 0;
    while (updated < size){
      while (idx < selectedWords[sentNo].size()){
	if ( sentNo == stopSentNo && selectedWords[sentNo][idx].phrNo == stopPhrNo ) return;
  ...
  void CopyWords(const SemanticSimilarityModelState& state, const uint sentno, 
  ...
  uint AddWord(const uint sentno, const uint phrno, const AnchoredPhrasePair &app, const std::string pos, const int historySize){
      ...
      if ( posTags[sentno][wordno] == pos ){
        for (WordAlignment::const_iterator wit = wa.begin_for_source(j);
          ...
          if (b >=0){
            ...
            selectedWords[sentno].back().similarity = 
              MaxSimilarityWithHistory(sentno,selectedWords[sentno].size()-1,historySize);
  ...
  void UpdateSimilarityScore(const uint sentNo, const uint idx, const uint size){
    selectedWords[sentNo][idx].similarity = MaxSimilarityWithHistory(sentNo,idx,size);
  };

  float MaxSimilarityWithHistory(uint sentNo, const uint idx, const uint size){
    std::vector<SelectedWordVector*> history;
    GetHistory(history,sentNo,idx,size);
    float maxSim = 0;
    while (!history.empty()){
      SelectedWordVector *last = history.back();
      float sim = CosinusSimilarity(selectedWords[sentNo][idx].vec,last->vec);
  ...
  float CosinusSimilarity(const std::vector<float> &vec1,const std::vector<float> &vec2){
    ...
    float sim = 0;
    for (a = 0; a < size; a++) sim += vec1[a] * vec2[a] / len;
    // return log(sim);
    return sim;
  }
...
SemanticSimilarityModel::SemanticSimilarityModel(const Parameters &params) : logger_("SemanticSimilarityModel") {
...
void SemanticSimilarityModel::loadVectors(const std::string file_name){
  ...
  vocab = (char *)malloc((long long)words * MaxWordSize * sizeof(char));  
  M = (float *)malloc((long long)words * (long long)size * sizeof(float));
  if (M == NULL) {
    LOG(logger_, error, "Cannot allocate memory: " << 
        (long long)words * size * sizeof(float) / 1048576 << "MB, " 
...
FeatureFunction::State *SemanticSimilarityModel::initDocument(const DocumentState &doc, Scores::iterator sbegin) const {
...
void SemanticSimilarityModel::computeSentenceScores(const DocumentState &doc, uint sentno, Scores::iterator sbegin) const {
	*sbegin = Float(0);
}


FeatureFunction::StateModifications *SemanticSimilarityModel::estimateScoreUpdate(const DocumentState &doc, const SearchStep &step, const State *state,
		Scores::const_iterator psbegin, Scores::iterator sbegin) const {

  const SemanticSimilarityModelState *prevstate = dynamic_cast<const SemanticSimilarityModelState *>(state);
  ...
  while (it!=mods.end()){
    ...
    s->UpdateFutureScores(sentNo,wordIdx,HistorySize-1,nextModSentNo,nextModFrom);
  }
...
FeatureFunction::StateModifications *SemanticSimilarityModel::updateScore(const DocumentState &doc, const SearchStep &step, const State *state,
		FeatureFunction::StateModifications *estmods, Scores::const_iterator psbegin, Scores::iterator estbegin) const {
	return estmods;
}

FeatureFunction::State *SemanticSimilarityModel::applyStateModifications(FeatureFunction::State *oldState, FeatureFunction::StateModifications *modif) const {
